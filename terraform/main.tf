...
resource "aws_ec2_instance_state" "backend" {
  ...
  state       = "stopped"
  depends_on = [ null_resource.backend ] #stop the server only when null resource provisioning completed
}
...
resource "aws_lb_target_group" "backend" {
  ...
  vpc_id   = data.aws_ssm_parameter.vpc_id.value
   ...
   health_check {
    path                = "/health"  #/health is already there in index.js in code
    port                = 8080
    protocol            = "HTTP"
    healthy_threshold   = 2
...
resource "aws_autoscaling_group" "backend" {
  ...
  desired_capacity          = 1
  target_group_arns         = [aws_lb_target_group.backend.arn]  #this is list so using [] 
